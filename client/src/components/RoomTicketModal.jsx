...
export default function RoomTicketModal({ 
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: '#ffffff', borderRadius: '28px', width: '100%', maxWidth: '720px', maxHeight: '94vh', overflowY: 'auto', boxShadow: '0 30px 70px rgba(0,0,0,0.35)', position: 'relative' }}>
        ...
        <div style={{ padding: '28px' }}>
          ...
          <div style={{ background: '#ffffff', border: '2px solid #e2e8f0', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.06)', position: 'relative' }}>
            ...
            <div style={{ padding: '24px', background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', borderBottom: '2px dashed #cbd5e1', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '20px', alignItems: 'center' }}>
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: '800', color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px' }}>
                  Assigned Room Suite
                </span>
                <div style={{ fontSize: '1.8rem', fontWeight: '900', color: '#0f172a', letterSpacing: '-0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>{roomNumber}</span>
                </div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#e0f2fe', color: '#0369a1', padding: '4px 10px', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: '700', marginTop: '6px' }}>
                  <ShieldCheck size={14} /> Guaranteed Direct Host Allocation
                </div>
              </div>

              {/* Digital Door PIN */}
              <div style={{ background: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.04)', textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '0.75rem', color: '#64748b', fontWeight: '700', textTransform: 'uppercase' }}>
                ...
                <div style={{ fontSize: '0.68rem', color: '#94a3b8' }}>Tap PIN at smart-lock door</div>
              </div>
            </div>
            ...
            <div style={{ padding: '24px', display: 'grid', gridTemplateColumns: '1fr 140px', gap: '20px', alignItems: 'center' }}>
              <div>
                ...
                {/* Dates & Guest Specs */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', background: '#f8fafc', padding: '12px 16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                  <div>
                    <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '700', textTransform: 'uppercase' }}>Check-In</div>
                    <div style={{ fontSize: '0.88rem', fontWeight: '800', color: '#0f172a' }}>
                      {inDate.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </div>
                    <div style={{ fontSize: '0.72rem', color: '#059669', fontWeight: '700' }}>From 2:00 PM</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '700', textTransform: 'uppercase' }}>Check-Out</div>
                    <div style={{ fontSize: '0.88rem', fontWeight: '800', color: '#0f172a' }}>
                      {outDate.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </div>
                    <div style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: '700' }}>Until 11:00 AM</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '20px', marginTop: '14px', fontSize: '0.82rem' }}>
                  <div>
                    ...
                    <strong style={{ color: '#0f172a' }}>{booking.guestName || 'Valued Guest'}</strong>
                  </div>
                  <div>
                    <span style={{ color: '#64748b' }}>Occupancy: </span>
                    <strong style={{ color: '#0f172a' }}>{booking.guests || 1} guest{(booking.guests || 1) > 1 ? 's' : ''}</strong>
                  </div>
                  <div>
                    <span style={{ color: '#64748b' }}>Paid: </span>
                    <strong style={{ color: '#059669' }}>₹{totalAmount.toLocaleString('en-IN')}</strong>
                  </div>
                </div>
              </div>

              {/* Digital Check-in QR Code */}
              <div style={{ textAlign: 'center', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '12px', boxShadow: '0 4px 14px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ fontSize: '0.68rem', fontWeight: '700', color: '#64748b', marginTop: '6px' }}>
                  Scan at Front Desk
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', padding: '12px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.75rem', color: '#64748b' }}>
